...
express.get("/exchangeSsoTokenForOboToken", async (req, res) => {
    ...
    const aadAuthorizeEndpoint = `https://login.microsoftonline.com/${jwtDecode<any>(ssoToken).tid}/oauth2/v2.0/authorize?`;

    // build body of request to obtain an access token using the OAuth2 OBO flow
    const aadAuthorizeParams = {
        client_id: clientId,
        response_type: "code",
        redirect_uri: process.env.PUBLIC_HOSTNAME + "/auth-end",
        response_mode: "query",
    ...
    const aaAuthorizeRequest = Object.keys(aadAuthorizeParams)
